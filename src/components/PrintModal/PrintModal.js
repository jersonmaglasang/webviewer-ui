    ...
    return (
      ...
      >
        <div className="container" onClick={e => e.stopPropagation()}>
          ...
          {this.state.allowWatermarkModal && <button data-element="applyWatermark" className="apply-watermark" onTouchStart={() => this.setWatermarkModalVisibility(true)} onClick={() => this.setWatermarkModalVisibility(true)}>{t('option.print.addWatermarkSettings')}</button> }
          <div className="buttons">
            ...
            <button className="button" onClick={this.createPagesAndPrint} onTouchStart={this.createPagesAndPrint} disabled={count > -1}>{t('action.print')}</button>
          </div>
        </div>
      </div>
      </>
